...
if (!EVM_PRIVATE_KEY || !EVM_RPC_URL) {
    console.error("WALLET_PRIVATE_KEY and RPC_PROVIDER_URL environment variables are required for EVM");
    process.exit(1);
}
...
async function main() {
    try {
        ...
        const getTokenInfoByTickerToolEVM = evmTools.find((tool) => tool.name === "get_token_info_by_ticker");

        if (!getTokenInfoByTickerToolEVM) {
        ...
        for (const ticker of evmTokens) {
            console.log(`\nLooking up ${ticker}...`);
            try {
                const tokenInfo = await getTokenInfoByTickerToolEVM.execute({ ticker });
        ...
        const getTokenInfoBySymbolToolSolana = solTools.find((tool) => tool.name === "get_token_info_by_ticker");

        if (!getTokenInfoBySymbolToolSolana) {
            throw new Error("get_token_info_by_ticker tool not found in plugin");
        }
