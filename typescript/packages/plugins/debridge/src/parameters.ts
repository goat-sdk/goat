...
export class getBridgeQuoteParametersSchema extends createToolParameters(
    z.object({
        ...
        srcChainTokenIn: z.string().regex(EVM_ADDRESS_REGEX, "Token address must be '0x0' for native token (ETH) or a valid EVM address"),

        /** Amount of tokens to bridge in base units (e.g., wei for ETH) */
        srcChainTokenInAmount: z.union([
            z.literal("auto"),
            z.string().pipe(
        ...
        prependOperatingExpenses: z.boolean()
            .optional()
            .default(true)
            .describe("Whether to include operating expenses in the transaction. Always true for native token transfers."),
    }),
) {}
...
export class createBridgeOrderParametersSchema extends createToolParameters(
    z.object({
        /** Chain ID of the source blockchain */
        srcChainId: chainIdSchema.describe("Source chain ID (e.g., '1' for Ethereum, '56' for BSC) where the cross-chain swap will start"),

        /** Token address on the source chain */
        srcChainTokenIn: z.string()
            .refine(
        ...
        srcChainTokenInAmount: z.string()
            .pipe(
                z.coerce.number().positive().int()
                .transform(String)
            )
        ...
        dstChainTokenOutRecipient: z.string()
            .refine(
                (val) => EVM_ADDRESS_REGEX.test(val) || SOLANA_ADDRESS_REGEX.test(val),
        ...
        srcChainOrderAuthorityAddress: z.string()
        ...
        srcChainRefundAddress: z.string()
        ...
        dstChainOrderAuthorityAddress: z.string()
            .refine(
                (val) => EVM_ADDRESS_REGEX.test(val) || SOLANA_ADDRESS_REGEX.test(val),
 ...
 */
export class getSupportedChainsParametersSchema extends createToolParameters(
    z.object({}),
) {}
