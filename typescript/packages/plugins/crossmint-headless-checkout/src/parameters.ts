	...
	return z.object({
		...
		payment: z
			.object({
				...
				payerAddress: z
					.string()
					.describe("The address that will pay for the transaction"), // TODO: This required for now, as this will create and buy the order in 1 tool
				receiptEmail: z
					.string()
					.optional()
					.describe("Optional email to send payment receipt to"),
			})
			.describe(
				...
				z.union([
					...
					z.object({
						productLocator: z
							.string()
							.describe(
								"The product locator. Ex: 'amazon:<amazon_product_id>', 'amazon:<asin>'",
							),
						...(callDataSchema ? { callData: callDataSchema } : {}),
					}),
				]),
...
export const physicalAddressSchema = z
	.object({
		...
		postalCode: z
			.string()
			.min(1, "Postal/ZIP code is required for physical address"),
		country: z
			.string()
	...
	.describe(
		"International mailing address using ISO 3166-1 alpha-2 country codes",
	);
