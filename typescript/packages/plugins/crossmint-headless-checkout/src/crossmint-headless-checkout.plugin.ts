	...
	async getTools(walletClient: EVMWalletClient) {
		...
		return [
			...
			createTool(
				...
				async (params) => {
					const res = await this.crossmintApiClient.post(
						"/api/2022-06-09/orders",
						{
					...
					const isRequiresPhysicalAddress =
						order.quote.status === "requires-physical-address";
					if (isRequiresPhysicalAddress) {
						throw new Error("recipient.physicalAddress is required");
					}

					const serializedTransaction =
						order.payment.preparation != null &&
					...
					const transaction = parseTransaction(
						serializedTransaction as `0x${string}`,
					);
...
export const crossmintHeadlessCheckout = (
	crossmint: Crossmint,
