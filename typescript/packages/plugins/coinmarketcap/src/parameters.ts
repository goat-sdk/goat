...
const convertSchema = z.object({
    convert: z
        .string()
        .optional()
        .describe("Comma-separated cryptocurrency or fiat currency symbols to convert market data into"),
});
...
export const cryptocurrencyListingsParametersSchema = paginationSchema
    .merge(convertSchema)
    .merge(sortSchema)
    .extend({
        ...
        tag: z.string().optional().describe("Filter by cryptocurrency tags (e.g., 'defi', 'filesharing')"),
        aux: z
            .string()
            .optional()
            .describe(
                "Comma-separated list of supplemental data fields: platform,tags,date_added,circulating_supply,total_supply,max_supply,cmc_rank,num_market_pairs",
            ),
    });
...
export const cryptocurrencyQuotesLatestParametersSchema = z.object({
    ...
    convert: z
        .string()
        .optional()
        .describe("Comma-separated fiat or cryptocurrency symbols to convert market data into"),
    aux: z.string().optional().describe("Comma-separated list of supplemental data fields"),
});

// Exchange endpoints parameters
export const exchangeListingsParametersSchema = paginationSchema
    .merge(convertSchema)
    .merge(sortSchema)
    .extend({
        market_type: z.enum(["all", "spot", "derivatives"]).optional().describe("Type of exchange markets to include"),
        aux: z
            .string()
            .optional()
            .describe(
                "Comma-separated list of supplemental data fields: num_market_pairs,traffic_score,rank,exchange_score,liquidity_score",
            ),
    });
