...
import type { CapacityCreditsRes, MintCapacityCreditsRes, SessionSigsMap } from '@lit-protocol/types';
...
export async function createCapacityCreditDelegationAuthSig(litNodeClient: LitNodeClient, ethersWallet: ethers.Wallet, capacityTokenId: string): Promise<CapacityCreditsRes> {
  return litNodeClient.createCapacityDelegationAuthSig({
...
export async function getPKPSessionSigs(litNodeClient: LitNodeClient, pkpPublicKey: string, ethersWallet: ethers.Wallet, capacityTokenId: string): Promise<SessionSigsMap> {
    return litNodeClient.getPkpSessionSigs({
      pkpPublicKey,
      capabilityAuthSigs: [
        // @ts-expect-error Type 'CapacityCreditsRes' is missing the following properties from type 'AuthSig'
        // This is a type bug in the Lit SDK
      ...
      authMethods: [
        await EthWalletProvider.authenticate({
          signer: ethersWallet,
          // @ts-expect-error Type 'LitNodeClient' is missing the following properties from type 'LitNodeClient'
          // This is a type bug in the Lit SDK
          litNodeClient,
...
export async function generateWrappedKey(litNodeClient: LitNodeClient, pkpSessionSigs: SessionSigsMap, network: "evm" | "solana", memo?: string): Promise<GeneratePrivateKeyResult> {
    return await generatePrivateKey({
        litNodeClient,
        pkpSessionSigs,
        network,
