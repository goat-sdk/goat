...
export class SafeWalletClient extends EVMSmartWalletClient {
	...
	async #createAndExecuteTransaction(
		...
		if (!transaction)
			throw new SafeWalletError("Failed to create Safe transaction");

		return transaction;
	}

	async sendBatchOfTransactions(
	...
	async resolveAddress(address: string): Promise<`0x${string}`> {
		...
		try {
			const resolvedAddress = (await this.#client
	...
	async signMessage(message: string) {
		...
		try {
			...
			const signature = await this.#safeAccount.protocolKit.signMessage(
				safeMessage,
