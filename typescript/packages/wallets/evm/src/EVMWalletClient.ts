...
export abstract class EVMWalletClient extends WalletClientBase {
	...
	constructor({
		...
		enableSend = true,
	}: { tokens?: Token[]; enableSend?: boolean } = {}) {
		super();
		this.tokens = tokens;
	...
	async balanceOf(
		...
		if (options?.tokenAddress) {
			...
			try {
				...
				console.error(
					`Failed to fetch ERC20 balance for ${tokenAddress}: ${error}`,
				);
				throw new Error(
	...
	async getTokenInfoByTicker(ticker: string) {
		...
		const token = this.tokens.find(
			(t) => t.symbol.toUpperCase() === upperCaseTicker,
		);
		...
		if (token) {
			...
			if (!contractAddress) {
				throw new Error(
					`Token ${ticker} configured but not found on chain ${chain.id}`,
				);
			}
		...
		throw new Error(
			`Token with ticker ${ticker} not found or not configured for chain ${chain.id}`,
		);
	}
	...
	async getTokenAllowance(
		...
		try {
			...
			throw Error(
				`Failed to fetch allowance: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	}
	...
	async approve(params: ApproveParameters): Promise<{ hash: string }> {
		if (!this.enableSend) {
			throw new Error(
				"Approval operations are disabled for this wallet instance.",
			);
		}
		try {
			...
			if (!/^\d+$/.test(amount)) {
				throw new Error(
					`Invalid base unit amount format for approval: ${amount}`,
				);
			}
			return this.sendTransaction({
				to: tokenAddress,
	...
	async transferFrom(
		...
		try {
			...
			if (!/^\d+$/.test(amount)) {
				throw new Error(
					`Invalid base unit amount format for transferFrom: ${amount}`,
				);
			}
			return this.sendTransaction({
				to: tokenAddress,
			...
			throw Error(
				`Failed to transfer from: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	}
	...
	override getCoreTools(): ToolBase[] {
		const baseTools = super.getCoreTools(); // Get tools from WalletClientBase

		const evmTools: ToolBase[] = [
			// Tool for signTypedData (ensure EVMTypedData structure allows zod schema creation)
			createTool(
				{
					name: "sign_typed_data_evm", // Make name more specific
					description: "Sign an EIP-712 typed data structure (EVM)",
					// Example schema - THIS NEEDS TO MATCH THE ACTUAL EVMTypedData structure
					parameters: z
						.object({
							domain: z.object({}).passthrough(), // Define domain fields
							types: z.object({}).passthrough(), // Define types structure
							primaryType: z.string(),
							message: z.object({}).passthrough(), // Define message structure
						})
						.passthrough(),
				},
				// Cast parameters safely - ensure EVMTypedData type is correct
				(data: EVMTypedData) => this.signTypedData(data),
			),
			// Tools for ERC20 specific actions
			createTool(
				{
			...
			createTool(
				...
				(params) => this.revokeApproval(params),
			),
			createTool(
				{
				...
				(params) => this.transferFrom(params),
			),
		];
